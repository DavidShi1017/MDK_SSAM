...
export default class {
    ...
    static async UpdatePartnerControls(context, filters) {
        ...
        const requiredMark = libPersona.isFieldServiceTechnician(context) ? '*' : '';
    ...
    static setAddFromSuboperationFlag(context, FlagValue) {
        libCom.setStateVariable(context, 'NotificationFromSubOperation', FlagValue, 'SubOperationDetailsPage');
    }
    ...
    static getAddFromSuboperationFlag(context) {
        let result = libCom.getStateVariable(context, 'NotificationFromSubOperation', 'SubOperationDetailsPage');
        if (result) {
            return result;
        } else {
            return false;
