...
export default async function InspectionCharacteristicsPageMetadataGenerator(clientAPI) {
    ...
    count = 0;
    sectionIndex = 0;
    var page = clientAPI.getPageProxy().getPageDefinition('/SAPAssetManager/Pages/GenericPage.page');
    let entityset = '';
    ...
    if (binding['@odata.type'] === '#sap_mobile.InspectionLot') {
        if (binding.InspectionPoints_Nav && binding.InspectionPoints_Nav.length > 0) {
            ...
            CommonLibrary.setStateVariable(context, binding.OrderId + "-" + binding.InspectionLot, binding.WOHeader_Nav);
        } else {
